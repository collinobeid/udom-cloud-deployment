...
class ChattyLLMTaskListener implements IEventListener {
	...
	public function handle(Event $event): void {
		...
		if (preg_match('/^chatty-llm:(\d+)/', $customId, $matches)) {
			...
			try { $session = $this->sessionMapper->getUserSession($task->getUserId(), $sessionId); } catch (\OCP\AppFramework\Db\DoesNotExistException $e) { $this->logger->info("ChattyLLM session not found, ignoring TaskSuccessfulEvent.", ["sessionId" => $sessionId]); return; }
