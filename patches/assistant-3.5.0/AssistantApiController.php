...
class AssistantApiController extends OCSController {
	 ...
	 * @return DataResponse
	 */
	public function scheduleTask(string $identifier, string $taskTypeId, array $input): DataResponse {
		$task = new \OCP\TaskProcessing\Task($taskTypeId, $input, 'assistant', $this->userId, $identifier);
		try {
			$this->taskProcessingManager->scheduleTask($task);
			return new DataResponse(['task' => $task->jsonSerialize()]);
		} catch (\Exception $e) {
